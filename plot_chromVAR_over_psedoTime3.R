...
if(T){
  #fn <- paste0("hmp.pt.alpha.smooth.label",with_lab,".png")
  fn <- paste0("hmp.pt.alpha.smooth.label",with_lab,".pdf")
  colr <- adjustcolor( brewer.pal(9,"Set1")[1:2],alpha.f = 0.2)
  #png(filename =fn,height =4*109/66,width = 6,units = 'in',res = 300)
  pdf(file =fn,height =4*109/66,width = 6)
  par(mgp=c(1,.3,0))
  if(with_lab==T){
  ...
  plot(pd.anno.a$pt,rep(1,nrow(pd.anno.a)),col=colr[pd.anno.a$subtype],xaxs="i",
       yaxt='n',ylab="",xaxt='n',pch=20,cex=2,bty="n")
  
  if(with_lab){ 
    par(mar=c(0.5,4,0,0)+0.1)}else{
    par(mar=c(0.5,.4,0,0)+0.1)
  }

    for(i in 1:2){
    if(i==2) par(mar=c(2.5,4,0,0)+0.1)
